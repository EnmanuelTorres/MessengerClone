...
final class RegisterViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let gesture = UITapGestureRecognizer(target: self, action: #selector(didTapChabgeProfilePic))
        
        imageView.addGestureRecognizer(gesture)
    }
    
    @objc private func didTapChabgeProfilePic(){
        pressenPhotoActionSheet()
    }
    ...
    @objc private func registerButtonTapped(){
      ...
      //  lastNameField.resignFirstResponder()
        ...
        DatabaseManager.shared.userExists(with: email) { [weak self] exists in
            ...
            guard !exists else {
                //user already exists
                strongSelf.alerUserLoginError(message: "Looks like a user account for that email address already existed.")
                return
            }
            
            FirebaseAuth.Auth.auth().createUser(withEmail: email, password: password) { AuthDataResult, Error in
                guard AuthDataResult != nil, Error == nil else {
                    print("Error crearing user ")
                    return
                }
...
extension RegisterViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func pressenPhotoActionSheet(){
        let actionSheet = UIAlertController(title: "Profile Picture",
                                            message: "How would you like to seecta picture?",
